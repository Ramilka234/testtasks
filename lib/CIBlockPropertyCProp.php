...
class CIBlockPropertyCProp
{
    ...
    public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
	    ...
	    else if($arItem['TYPE'] === 'html_editor'){
    ...
    // Открываем буфер для захвата вывода HTML-редактора
    	ob_start();

    // Добавляем встроенный редактор Bitrix
    	CFileMan::AddHTMLEditorFrame(
        	$strHTMLControlName['VALUE'] . '[' . $code . ']', // Имя поля
        	$v,                                               // Значение поля
        	$strHTMLControlName['VALUE'] . '_TYPE_' . $code,  // Имя для типа контента (html/text)
        	strlen($v) ? "html" : "text",                     // Тип содержимого по умолчанию (html или текст)
        	array(
        	    'height' => 200, // Высота редактора (можно изменить по необходимости)
        	)
    	);	

    // Сохраняем вывод редактора в переменную
    	$result .= ob_get_clean();
    ...
    private static function getOptionList($selected = 'string')
    {
	    ...
	    'html_editor' => Loc::getMessage('IEX_CPROP_FIELD_TYPE_HTML_EDITOR')
        ];
        ...
        foreach ($arOption as $code => $name){
            ...
            if($code === $selected){
             ...
             $result .= '<option value="' . htmlspecialchars($code) . '" ' . $s . '>' . htmlspecialchars($name) . '</option>';
        }
